...
"""
from pysgg.data.datasets.evaluation.vg.vg_stage_eval_utils import (
    boxlist_iou,
...
"""
from lavis.tasks.utils import intersect_2d, argsort_desc, bbox_overlaps
...
def _compute_pred_matches(
    ...
    for i in range(pred_boxes.shape[0]):
        false_positives[i] = len(pred_to_gt[i]) - true_positives[i]
    # Sort predictions based on confidence scores and take top-k
    sorted_indices = np.argsort(-pred_boxes[:, -1])

    return pred_to_gt, true_positives[sorted_indices], false_positives
...
class SGRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode, ensemble_model=None):
        self.ensemble_model = ensemble_model
        self.result_dict[mode + f"_{self.type}"] = {20: [], 50: [], 100: []}
        self.result_dict[mode + '_gr_recall'] = {20: [], 50: [], 100: []}
        self.result_dict[mode + '_recall_per_rel'] = {20: [], 50: [], 100: []}
        self.result_dict[mode + '_gr_recall_per_rel'] = {20: [], 50: [], 100: []}
        self.result_dict[mode + f"_precision"] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode, gr=False):
        result_str = "SGG eval: "
        if gr:
            for k, v in self.result_dict[mode + "_gr_recall"].items():
                result_str += "  gr_R @ %d: %.4f; " % (k, np.mean(v))
            result_str += " for mode=%s, type=gr_Recall(Main)." % mode
            result_str += "\n"
            return result_str
        else:
    ...
    def calculate_recall(self, global_container, local_container, mode, gr=False):
        #pred_rel_inds = local_container["pred_rel_inds"]
        #rel_scores = local_container["rel_scores"]
        gt_rels = local_container["gt_triplets"]
        ...
        #pred_scores = local_container['rel_scores']#rel_scores #[:, 1:].max(1)
       ...
       # gt_triplets, gt_triplet_boxes, _ = _triplet(gt_rels, gt_classes, gt_boxes)
        ...
        pred_triplets = local_container["pred_triplets"]
        gt_triplet_boxes = local_container["gt_triplet_boxes"]
        pred_triplet_boxes = local_container["pred_triplet_boxes"]

        pred_to_gt, tp, fp = _compute_pred_matches(
            gt_triplets,
            pred_triplets,
            gt_triplet_boxes,
            pred_triplet_boxes,
        ...
        if gr:
            re_dict = self.result_dict[mode + "_gr_recall"]
        else:
            re_dict = self.result_dict[mode + "_recall"]
        for k in re_dict:
            # Use an empty array as the initial value for reduce
            initial_value = np.array([])
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k], initial_value)
            ...
            if gr:
                self.result_dict[mode + '_gr_recall_per_rel'][k].append(rec_per_rel)
                self.result_dict[mode + "_gr_recall"][k].append(rec_i)
            else:
                self.result_dict[mode + '_recall_per_rel'][k].append(rec_per_rel)
                self.result_dict[mode + "_recall"][k].append(rec_i)
            #p_at_k = np.sum(tp) / (k + 1e-10)
            #self.result_dict[mode + f"_precision"][k] = p_at_k
...
"""
Given Ground Truth Object-Subject Pairs
...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def get_class_level_recall(self, mode, ensemble_model=None):
        self.ensemble_model = ensemble_model
        return self.rel_name_list, self.result_dict[mode + "_mean_recall_list"][100]
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + "_gr_mean_recall"] = {20: 0.0, 50: 0.0, 100: 0.0}
        ...
        self.result_dict[mode + "_gr_mean_recall_collect"] = {
            20: [[] for i in range(self.num_rel)],
            50: [[] for i in range(self.num_rel)],
            100: [[] for i in range(self.num_rel)],
        }
        self.result_dict[mode + "_mean_recall_list"] = {20: [], 50: [], 100: []}
        self.result_dict[mode + "_gr_mean_recall_list"] = {20: [], 50: [], 100: []}
        #self.result_dict[mode + "_macro_recall"] = {20: 0.0, 50: 0.0, 100: 0.0}


    def generate_print_string(self, mode, gr=False):
        result_str = "SGG eval: "
        if gr:
            rel_dict = self.result_dict[mode + "_gr_mean_recall"]
        else:
            rel_dict = self.result_dict[mode + "_mean_recall"]
        for k, v in rel_dict.items():  #self.result_dict[mode + "_mean_recall"].items():
            if gr:
                result_str += "  gr_mR @ %d: %.4f; " % (k, float(v))
            else:
        ...
        if self.print_detail:
            if gr:
                result_str += "Per-class gr_recall@50: \n"
                for n, r in zip(
                        self.rel_name_list, self.result_dict[mode + "_gr_mean_recall_list"][50]
                ):
                    result_str += "({}:{:.4f}) ".format(str(n), r)
                result_str += "\n"
                result_str += "Per-class gr_recall@100: \n"
                for n, r in zip(
                        self.rel_name_list, self.result_dict[mode + "_gr_mean_recall_list"][100]
                ):
                    result_str += "({}:{:.4f}) ".format(str(n), r)
                result_str += "\n\n"
            else:
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode, gr=False):
        pred_to_gt = local_container["pred_to_gt"]
        gt_rels = local_container["gt_triplets"]
        # Use an empty array as the initial value for reduce
        initial_value = np.array([])
        if gr:
            for k in self.result_dict[mode + "_gr_mean_recall_collect"]:
            # the following code are copied from Neural-MOTIFS
                match = reduce(np.union1d, pred_to_gt[:k], initial_value)
                ...
                for n in range(self.num_rel):
                    if recall_count[n] > 0:
                        self.result_dict[mode + "_gr_mean_recall_collect"][k][n].append(
    ...
    def calculate_mean_recall(self, mode, gr=False):
        if gr:
            for k, v in self.result_dict[mode + "_gr_mean_recall"].items():
                sum_recall = 0
                num_rel_no_bg = self.num_rel - 1
                for idx in range(num_rel_no_bg):
                    if len(self.result_dict[mode + "_gr_mean_recall_collect"][k][idx + 1]) == 0:
                        tmp_recall = 0.0
                    else:
                        tmp_recall = np.mean(
                            self.result_dict[mode + "_gr_mean_recall_collect"][k][idx + 1]
                        )
                    self.result_dict[mode + "_gr_mean_recall_list"][k].append(tmp_recall)
                    sum_recall += tmp_recall

                self.result_dict[mode + "_gr_mean_recall"][k] = sum_recall / float(num_rel_no_bg)
        else:
